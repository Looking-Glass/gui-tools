...
namespace HoloPlay
{
    ...
    public class Quilt : MonoBehaviour
    {
...
#if UNITY_EDITOR
        public void Force2DMode() {
            renderIn2D = true;
            Update2DMode();
        }

        public void Update2DMode()
        {
        ...
        public IEnumerator Screenshot3D(string path = null)
        {
            ...
            string fullName = "";
            if (string.IsNullOrEmpty(path) && !Misc.GetNextFilename(Path.GetFullPath("."), Application.productName + "_" + SerializeTilingSettings(tiling), ".png", out fullName, out path))
            {
                Debug.LogWarning(Misc.debugLogText + "Couldn't save screenshot");
            }
            else
